...
def AreaConditions(blob, wheel_x):
    global side
    # print('hellllllllllllllo' , blob.pixels() / blob.area())
    if side == 'right':
        return (
...
def filter_color(blob):
    ...
    # if 0.17 <= blob.area() / 19200 :
    #     return False
    # if blob.area() / 19200 <= 0.035:
    #     return False
    else:
        return True
...
def FoundVictim(letter):
    ...
    print("Detected victim", letter, "for", victims_detections[letter], "time(s)")
...
while True:
    ...
    region = {'right': (wheel_x , 0 , frame_width, wall-2),
                'left':(0 , 0 , frame_width , wall-2)}
    ...
    is_color = False
    try:
        blbs = img.find_blobs(THRESHOLD, area_threshold=400, pixels_threshold=400, roi= region_of_interest , merge=True)
    ...
    if is_color == False:
        ...
        img.to_grayscale()
        img.gaussian(2)
        img.mean(5, threshold=True, offset=16, invert=True)  # change ----> offset
        ...
        for vic in vic1:
            ...
            if AreaConditions(vic, wheel_x):
                ...
                # for l in img.find_lines(threshold=2900 , theta_margin=10, rho_margin=20):

                for l in img.find_lines(threshold=2600, theta_margin=10, rho_margin=20):

                    lines.append(l.theta())
                try:
                    if lines[0] != 90 or lines[0] !=0 :
                        img = img.rotation_corr(0, 0, -(lines[0]))
        ...
        if is_blob:
            ...
            for i in range(HSU_MAT_X):
                for j in range(HSU_MAT_Y):
                    ...
                    binary_matrix[i, j] = 1 if avg_color > 50 else 0
                    print(avg_color)
            print(binary_matrix)
